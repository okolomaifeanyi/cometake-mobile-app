...
class WishlistDatasource {
  ...
  Future<void> addToWishlist(String productId) async {
    final userId = _client.auth.currentUser?.id;
    if (userId == null) {
      throw const AuthException('Not authenticated while adding wishlist item');
    }
    try {
      ...
      if (kDebugMode) {
        debugPrint(
          '[Wishlist] add failed product=$productId user=$userId '
      ...
      if (kDebugMode) {
        debugPrint(
          '[Wishlist] add auth failed product=$productId user=$userId '
          'message=${e.message}',
        );
      }
      rethrow;
      ...
      if (kDebugMode) {
        debugPrint(
          '[Wishlist] add unexpected failure product=$productId user=$userId '
          'error=$e',
        );
      }
      rethrow;
  ...
  Future<void> removeFromWishlist(String productId) async {
    final userId = _client.auth.currentUser?.id;
    if (userId == null) {
      throw const AuthException('Not authenticated while removing wishlist item');
    }
    try {
      await _client
          .from('core_saveditems')
      ...
      if (kDebugMode) {
        debugPrint(
          '[Wishlist] remove failed product=$productId user=$userId '
          'code=${e.code} message=${e.message} details=${e.details} '
          'hint=${e.hint}',
        );
      }
      ...
      if (kDebugMode) {
        debugPrint(
          '[Wishlist] remove auth failed product=$productId user=$userId '
          'message=${e.message}',
        );
      }
      rethrow;
      ...
      if (kDebugMode) {
        debugPrint(
          '[Wishlist] remove unexpected failure product=$productId '
          'user=$userId error=$e',
        );
      }
      rethrow;
